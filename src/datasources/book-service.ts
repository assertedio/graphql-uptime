...
export class BookService extends DataSource {
  ...
  getAuthorBooks(authorId) {
    const filteredBooks = bookAuthors.filter((bookAuthor) => bookAuthor.authorId === authorId).map((bookAuthor) => findBook(bookAuthor.bookId));
    return Promise.resolve(filteredBooks);
  }

  createAuthor(authorInput) {
    const author = { ...authorInput, id: newId() };
    authors.push(author);
  ...
  getPublisherBooks(publisherId) {
    return Promise.resolve(books.filter((book) => book.publisherId === publisherId));
  }

  createPublisher(publisherInput) {
    const publisher = { ...publisherInput, id: newId() };
  ...
  getBookAuthors(bookId) {
    const filteredAuthors = bookAuthors.filter((bookAuthor) => bookAuthor.bookId === bookId).map((bookAuthor) => findAuthor(bookAuthor.authorId));
    return Promise.resolve(filteredAuthors);
  }

  createBook(bookInput, publisherId) {
    const book = { ...bookInput, id: newId(), publisherId };
    books.push(book);
  ...
  updateBook(bookId, bookInput, publisherId) {
    ...
    if (book) {
      book = Object.assign(book, bookInput, { publisherId });
    }
    return Promise.resolve(book);
  }
  ...
  setBookAuthors(bookId, authorIds) {
    ...
    if (book) {
      ...
      bookAuthors = bookAuthors.filter((bookAuthor) => bookAuthor.bookId !== bookId);
